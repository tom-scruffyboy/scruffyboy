...
- [x] **Thank-you page share ask** (stage 1 of referral) (Jul 18). WhatsApp share + copy-link buttons added to both thanks.html and confirmed.html (`js/share.js`, `.share` styles in site.css). WhatsApp leads (HK's default channel); copy-link is the fallback. Free, static, no tracking. Share text drives to scruffyboy.com with the founder-pricing hook.
- [x] **Make thanks.html work harder for the confirm click** (Jul 18). Reworked the sub copy so the confirm click is framed as the gate on the founder spot ("your founder spot isn't saved until you click the link inside — so do that first"). Kept the one Moose joke. (Whether to drop double opt-in entirely is still a Tom decision.)
- [x] **Confirmed-signup conversion event** (Jul 18, not from the source doc). Added to confirmed.html only: GA4 `sign_up_confirmed` + Meta `CompleteRegistration`. The existing `generate_lead`/`Lead` events fire at form submit — i.e. *before* the double opt-in — so ads were optimizing toward unconfirmed signups and true list growth was invisible. This fires only on the post-confirmation landing, so it measures real subscribers and unlocks retargeting the ~20% who don't confirm. Next step (Tom): set it as a conversion in GA4 + Meta Events Manager.
